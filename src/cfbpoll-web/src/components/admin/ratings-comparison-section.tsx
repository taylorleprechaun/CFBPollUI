...
import { RatingsComparisonTable } from './ratings-comparison-table';

interface RatingsComparisonSectionProps {
  runState: AlgorithmRunState<ExperimentalCalculateResponse>;
  season: number | null;
...
export function RatingsComparisonSection({
  runState,
  season,
  selectedVersions,
  token,
  week,
}: RatingsComparisonSectionProps) {
  ...
  const successfulEntries = selectedVersions
    ...
    .map((version) => ({ algorithmVersion: version, result: runState[version].result as ExperimentalCalculateResponse }));

  const contentId = `ratings-comparison-${season}-${week}`;
      ...
      <CollapsibleContent id={contentId} isOpen={expanded}>
        <div className="p-6 space-y-4">
          ...
          {successfulEntries.length > 0 && (
            <RatingsComparisonTable entries={successfulEntries} season={season} token={token} week={week} />
          )}
        </div>
      </CollapsibleContent>
    </div>
