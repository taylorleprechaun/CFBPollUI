...
describe('OverallRecordCard', () => {
  it('colors the win percentage when classes is provided', () => {
    render(
      <OverallRecordCard
        label="Winner"
        totals={{ correct: 3, incorrect: 1, push: 0 }}
        classes="bg-green-100 text-green-800"
      />
    );

    expect(screen.getByText('75.0%')).toHaveClass('bg-green-100');
  });

  it('does not color the win-loss record even when classes is provided', () => {
    render(
      <OverallRecordCard
        label="Spread"
        totals={{ correct: 8, incorrect: 6, push: 1 }}
        classes="bg-green-100 text-green-800"
      />
    );

    expect(screen.getByText('8-6-1')).not.toHaveClass('bg-green-100');
  });
  ...
  it('renders the win percentage uncolored when classes is omitted', () => {
    render(<OverallRecordCard label="Winner" totals={{ correct: 3, incorrect: 1, push: 0 }} />);

    expect(screen.getByText('75.0%')).not.toHaveClass('inline-block');
  });
